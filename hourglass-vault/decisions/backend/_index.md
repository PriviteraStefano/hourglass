...
| [[ADR-BE-018 — Direction & Org Settings Encoding]] | Plan plane (migrations 021-022): direction status matrix (draft/active/superseded/cancelled, supersede only via create-with-supersedes_id), derived done/lapsed/claimed spectrum, audit vocabulary (direction + org_settings actions), org_settings key vocabulary, claim over-subscription lock (FOR UPDATE + in-tx Σ in cents), supersede-of-claim-row inheritance, est_hours DECIMAL(8,2), settings CRUD literal routes | DIR-01..06, D-13-01..34, OQ1..OQ8 | Proposed |
