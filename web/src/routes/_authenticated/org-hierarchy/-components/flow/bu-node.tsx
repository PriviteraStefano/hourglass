...
import {ChevronDown, ChevronRight, Edit2, MoreVertical, Plus, Trash2, Users} from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
function BUNodeComponent(props: NodeProps) {
  const {
    unit, memberCount, totalMemberCount, childCount,
    isCollapsed, viewMode, members, membersLoading, isDragHovered,
    onAddSubUnit, onEdit, onDelete, onToggleCollapse,
  } = props.data as BUNodeData

  const levelLabel = unit.hierarchy_level === 0 ? 'Root' : `Level ${unit.hierarchy_level}`
  ...
  return (
    ...
    >
      <div className="absolute top-2 right-2 z-10" onClick={(e) => e.stopPropagation()}>
        <DropdownMenu>
          <DropdownMenuTrigger render={<Button variant="ghost" className="h-6 w-6 p-0 hover:bg-muted"/>}>
      ...
      <CardContent className="p-3 space-y-1">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            {(memberCount ?? 0) > 0 && (
              <Badge variant="outline" className="text-xs shrink-0 gap-1 px-1.5">
