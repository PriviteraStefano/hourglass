...
function AddMemberPopover({unitId}: { unitId: string }) {
  ...
  const {data: orgMembers, isLoading: orgMembersLoading} = useQuery(orgMembersQueryOpts)
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="p-0 w-64" align="end">
        <Command>
          ...
          <CommandList>
            <CommandEmpty>{orgMembersLoading ? 'Loading...' : 'No available members'}</CommandEmpty>
            <CommandGroup>
              {availableMembers.map((m) => (
                ...
                >
                  ...
                  <span className="ml-1 text-muted-foreground text-[10px]">{m.user_email}</span>
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
    ...
    <Sheet open={selectedUnit !== null} onOpenChange={(o) => !o && setSelectedUnit(null)}>
      <SheetContent className="w-[400px] sm:max-w-[400px] overflow-y-auto">
        {selectedUnit && (
          <>
            <SheetHeader>
              <BreadcrumbNav breadcrumbs={breadcrumbs} onNavigate={setSelectedUnit}/>
              <SheetTitle>{selectedUnit.name}</SheetTitle>
              <SheetDescription>
                <Badge variant="secondary">
                  {selectedUnit.hierarchy_level === 0 ? 'Root' : `Level ${selectedUnit.hierarchy_level}`}
                </Badge>
                <span className="ml-2 text-muted-foreground font-mono text-xs">{selectedUnit.code}</span>
              </SheetDescription>
            </SheetHeader>
