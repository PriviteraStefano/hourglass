...
import {CustomersApis, type CreateCustomerRequest, type UpdateCustomerRequest} from '@/api/customers'
  ...
  const handleSubmit = async (data: CustomerFormData) => {
    try {
      if (mode === 'edit' && editingCustomer) {
        ...
        await toast.promise(
          updateCustomer.mutateAsync({id: editingCustomer.id, data: updateData}),
          {
        ...
        await toast.promise(
          createCustomer.mutateAsync(createData),
          {
            ...
            render={({field, fieldState}) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldContent>
                  ...
                  <Input id="company_name" placeholder="Acme Corp" {...field} disabled={mode === 'edit' && !!editingCustomer?.is_internal} />
                  {mode === 'edit' && editingCustomer?.is_internal && (
                    <p className="text-xs text-muted-foreground mt-1">Company name is locked for internal customers</p>
                  )}
                </FieldContent>
              ...
              render={({field, fieldState}) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldContent>
                    ...
                    <Input id="phone" placeholder="+1 234 567 8900" {...field} />
                  </FieldContent>
                  <FieldError errors={[fieldState.error]} />
                </Field>
              )}
            ...
            render={({field, fieldState}) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldContent>
                  ...
                  <Textarea id="address" placeholder="123 Main St, City, Country" rows={2} {...field} />
                </FieldContent>
                <FieldError errors={[fieldState.error]} />
              </Field>
            )}
