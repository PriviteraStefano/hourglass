...
import {type EntryStatus, type Expense} from '@/types'
...
export function ExpenseCalendar() {
  ...
  const {month, date} = useSearch({from: "/_authenticated/expenses/"})
  ...
  const statusByDate = useMemo(() => {
    ...
    expenses?.forEach((expense: Expense) => {
      const dateStr = format(new Date(expense.entry_date), 'yyyy-MM-dd')
      const existing = map.get(dateStr)
      if (!existing || (statusPriority[expense.status] ?? 0) > (statusPriority[existing] ?? 0)) {
        map.set(dateStr, expense.status)
      }
    })
    return map
  }, [expenses])
