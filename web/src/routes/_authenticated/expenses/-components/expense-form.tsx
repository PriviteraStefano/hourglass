...
export function ExpenseForm({ date, onSaveDraft, onSubmit, isPending }: ExpenseFormProps) {
      ...
      {category === 'mileage' && (
        <div className="space-y-1">
          ...
          <Input
            ...
            min="0"
            value={kmDistance ?? ''}
            onChange={(e) => setKmDistance(e.target.value ? parseFloat(e.target.value) : undefined)}
            placeholder="Km"
          />
        </div>
      )}
