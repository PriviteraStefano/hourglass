...
export function ContractDetail() {
  ...
  const recalculateMileage = useMutation(ContractsApis.recalculateMileageMutationOpts)
  ...
  const handleToggleActive = async () => {
    try {
      ...
      toast.success(c.is_active ? 'Contract marked as inactive' : 'Contract marked as active')
    } catch {
        ...
        <CardContent className="space-y-3">
          {isEditing ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-2">
                <Label>KM Rate</Label>
                <Input type="number" step="0.01" value={formData.km_rate ?? ''}
                       onChange={e => setFormData({...formData, km_rate: parseFloat(e.target.value)})}/>
              </div>
              <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                ...
                <Select value={formData.governance_model} onValueChange={v => setFormData({
                  ...formData,
                  governance_model: v as Contract['governance_model']
                })}>
                  <SelectTrigger><SelectValue/></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="creator_controlled">Creator Controlled</SelectItem>
              ...
              <div className="flex items-center gap-2">
                <Checkbox id="is_shared" checked={formData.is_shared}
              ...
              <div className="grid gap-2">
                ...
                <Select value={formData.customer_id ?? ''}
                        onValueChange={v => setFormData({...formData, customer_id: v ? v : undefined})}>
              ...
              <div className="flex gap-2 pt-2">
                <Button onClick={handleSave} disabled={updateContract.isPending}>Save</Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span>Status</span>
                <Badge variant={c.is_active ? 'default' : 'outline'} className={c.is_active ? 'bg-green-500' : ''}>
                  {c.is_active ? 'Active' : 'Inactive'}
                </Badge>
              </div>
              <Button variant="outline" size="sm" onClick={handleToggleActive} disabled={updateContract.isPending}>
                Toggle
              </Button>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Input type="date" defaultValue={getDefaultDate()} id="recalc-date" className="w-[140px]"/>
                <Button
                  onClick={() => handleRecalculate((document.getElementById('recalc-date') as HTMLInputElement).value)}
            ...
            <div className="flex items-center justify-between">
              ...
              {hasTimeEntries &&
                  <span className="text-xs text-muted-foreground ml-2">Cannot delete - has time entries</span>}
            </div>
          </CardContent>
        </Card>
      )}
