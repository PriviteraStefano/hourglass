...
describe('ProjectsApis', () => {
  it('projectsQueryOpts calls GET /api/projects with scope', async () => {
    const mockProjects = [
      {
        id: 'p1', name: 'Project Alpha', type: 'billable' as const,
        contract_id: 'c1', governance_model: 'creator_controlled' as const,
  ...
  it('createProjectMutationOpts sends POST /api/projects with body', async () => {
    ...
    const mockProject = {
      id: 'p2', name: 'New Project', type: 'internal' as const,
      contract_id: 'c1', governance_model: 'unanimous' as const,
      is_shared: false, is_active: true, created_by_org_id: 'o1',
      created_at: '2025-01-01T00:00:00Z',
    }
    ...
    const result = await ProjectsApis.createProjectMutationOpts.mutationFn(projectData)
