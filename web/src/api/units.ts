...
export const unitDetailQueryOpts = (id: string) => queryOptions({
...
export const unitMembersQueryOpts = (unitId: string) => queryOptions({
  queryKey: ['units', 'members', unitId],
  queryFn: async () => {
    const data = await api<import('@/types/unit.ts').UnitMember[]>(`/units/${unitId}/members`)
