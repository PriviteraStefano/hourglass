...
const validateInvitationCodeQueryOpts = (code: string) =>
  queryOptions({
    queryKey: ["invitations", "code", code],
    queryFn: async () =>
      api<InvitationResponse>(`/invitations/validate/code/${code}`),
    enabled: !!code,
  });

const validateInvitationTokenQueryOpts = (token: string) =>
  queryOptions({
    queryKey: ["invitations", "token", token],
    queryFn: async () =>
      api<InvitationResponse>(`/invitations/validate/token/${token}`),
    enabled: !!token,
  });
...
const bootstrapCheckQueryOpts = queryOptions({
  ...
  queryFn: async () =>
    api<{ needs_bootstrap: boolean }>("/auth/bootstrap-check"),
  retry: false,
});
